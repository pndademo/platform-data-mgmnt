...
class DataHandler(APIHandler):
    ...
    def __read_parts__(self, path):
        ...
        raise Return(parts)
	#The function will call hdb read_arch to get list of archive files
    @run_on_executor
    def __read_arch__(self):
        hdb_datasets = self.db_conn.read_arch()
        logging.info("Following datasets were received from table %s", hdb_datasets)
        raise Return(hdb_datasets)
	#The function will call hdb restore_datasets to move files from archive to HDFS.
    @run_on_executor
    ...
    def __get_parts__(self, path):
        try:
            ...
            raise Return(value_return.value)
    #The function calls internal function read_arch.
    @coroutine
    def __get_arch__(self):
        try:
            ...
            raise Return(value_return.value)			
    #The function calls internal function restore_datasets_db.
    @coroutine
...
class UpdateDatasets(DataHandler):
    ...
    def put(self, dataset_id, **kwargs):
        ...
        :return:
        """
		#it checks whether put rrequest is to restore file from Archive to HDFS.
...
class ListDatasetsArch(DataHandler):
    ...
    """
     #it serves the URI /datasets/arch by calling internal function get_arch().
