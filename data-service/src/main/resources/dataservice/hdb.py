...
class HDBDataStore(object):
    ...
    def __init__(self, hdfs_host, hbase_host, hbase_port_no, table_name, repo_path, properties):
		...
		# the properties has configuration value to be used to get archive detail.
        self.properties = properties
        self.master_dataset = list()
		# the dictionary is used to track files that are being moved from archive to HDFS.
	self.progress_file = {}
    ...
    """ 
     It will connect to S3 or Swift to get list of archived files
    """	 
    def read_arch(self):
        """
        ...
        try:
            ...
            #s3conn.create_bucket(properties['container_name'], location=properties['s3_archive_region'])
                ...
                for i in s3location.get_all_keys():
                    ...
                    #if not file_name.endswith('/') and  not self.progress_file.has_key(file_name) and not file_name.find('_COPYING_'):
                    if not file_name.endswith('/') and  not self.progress_file.has_key(file_name):
                    	logging.info('Got File Name ####### %s', file_name)
                        if not '_COPYING_' in file_name:
                        	hdfs_dataset.append(item)
            else:
        ...
        return hdfs_dataset
    """ 
     It will connect to S3 or Swift to restore file from archive to HDFS.
    """	 
    def restore_datasets(self, request_data):
		...
		logging.info("Inside restore Datasets function request_data is %s", request_data)
		    ...
		    #logging.info("Inside for loop function")
				...
				if self.progress_file.has_key(fileName):
						logging.info("File %s already being moved to HDFS",fileName) 
				else:
						self.progress_file[fileName]=1
				...
				#logging.info("Inside for loop function5")
						...
						del self.progress_file[fileName]
						#print (path_str)
